import { createContext, useContext } from "react";

export type Lang = "fa" | "en";
export type Currency = "AFN" | "USD";

export const translations = {
  fa: {
    appName: "مدیریت ساختمان و املاک",
    appShort: "م.س.ا",
    dashboard: "داشبورد",
    construction: "ساختمان‌سازی",
    realEstate: "املاک و فروش",
    support: "پشتیبانی ساکنین",
    materials: "مواد و مصالح",
    workers: "کارگران",
    salaries: "معاشات",
    expenses: "مصارف",
    apartments: "آپارتمان‌ها",
    clients: "مشتریان",
    sales: "فروشات",
    requests: "درخواست‌های خدمات",
    settings: "تنظیمات",
    language: "زبان",
    currency: "اسعار",
    add: "افزودن",
    edit: "ویرایش",
    delete: "حذف",
    save: "ذخیره",
    cancel: "لغو",
    confirm: "تایید",
    name: "نام",
    description: "توضیحات",
    quantity: "تعداد",
    unit: "واحد",
    price: "قیمت",
    total: "مجموع",
    date: "تاریخ",
    status: "وضعیت",
    actions: "عملیات",
    search: "جستجو",
    noData: "هیچ داده‌ای موجود نیست",
    project: "پروژه",
    phone: "شماره تماس",
    address: "آدرس",
    role: "وظیفه",
    dailyWage: "مزد روزانه",
    daysWorked: "روزهای کاری",
    paid: "پرداخت شد",
    pending: "در انتظار",
    category: "دسته",
    amount: "مبلغ",
    block: "بلاک",
    floor: "طبقه",
    apartmentNo: "شماره آپارتمان",
    rooms: "اتاق",
    area: "مساحت (متر مربع)",
    available: "موجود",
    sold: "فروخته شده",
    reserved: "رزرو شده",
    client: "مشتری",
    apartment: "آپارتمان",
    salePrice: "قیمت فروش",
    paidAmount: "پرداخت شده",
    remaining: "باقی مانده",
    issue: "مشکل",
    priority: "اولویت",
    assignedTo: "اختصاص داده شده به",
    open: "باز",
    inProgress: "در حال اجرا",
    completed: "تکمیل شده",
    low: "کم",
    medium: "متوسط",
    high: "زیاد",
    welcome: "خوش آمدید",
    overview: "نمای کلی",
    totalMaterials: "کل مواد",
    totalWorkers: "کل کارگران",
    totalApartments: "کل آپارتمان‌ها",
    totalSales: "کل فروشات",
    openRequests: "درخواست‌های باز",
    monthlyExpenses: "مصارف ماهانه",
    recentActivity: "فعالیت اخیر",
    new: "جدید",
    none: "هیچ",
    inStock: "در انبار",
    supplier: "تامین‌کننده",
    notes: "یادداشت",
    paymentMethod: "روش پرداخت",
    cash: "نقد",
    bank: "بانک",
    purchaseDate: "تاریخ خرید",
    hireDate: "تاریخ استخدام",
    period: "دوره",
    payDate: "تاریخ پرداخت",
    selectWorker: "انتخاب کارگر",
    selectClient: "انتخاب مشتری",
    selectApartment: "انتخاب آپارتمان",
    requestedBy: "درخواست‌کننده",
    deleteConfirm: "آیا از حذف این مورد مطمئن هستید؟",
    buildings: "ساختمان‌ها",
    building: "ساختمان",
    buildingName: "نام ساختمان",
    floorsCount: "تعداد طبقات",
    floorPurpose: "کاربری طبقه",
    flatsPerFloor: "تعداد آپارتمان در هر طبقه",
    bathrooms: "حمام",
    kitchens: "آشپزخانه",
    livingRooms: "اتاق نشیمن",
    bedrooms: "اتاق خواب",
    residential: "مسکونی",
    parking: "پارکینگ",
    commercial: "تجاری",
    storage: "انبار",
    amenity: "خدماتی",
    mixed: "مختلط",
    generateBuilding: "ایجاد ساختمان کامل",
    floorConfiguration: "تنظیمات طبقات",
    flatConfiguration: "تنظیمات آپارتمان‌ها",
    defaultArea: "مساحت پیش‌فرض",
    defaultPrice: "قیمت پیش‌فرض",
    apply: "اعمال",
    floorNumber: "شماره طبقه",
    flats: "آپارتمان‌ها",
    addBuilding: "افزودن ساختمان",
    totalFlats: "کل آپارتمان‌ها",
    available_units: "واحدهای موجود",
    viewFlats: "مشاهده آپارتمان‌ها",
    flatTypes: "انواع آپارتمان",
    flatType: "نوع آپارتمان",
    addFlatType: "افزودن نوع آپارتمان",
    typeName: "نام نوع (مثلاً ۳ خوابه)",
    count: "تعداد",
    remove: "حذف",
    noFlatTypes: "برای این طبقه نوعی تعریف نشده است",
    applyToAll: "اعمال برای همه طبقات",
    appliedToAll: "تنظیمات برای همه طبقات اعمال شد",
    floorLabel: "برچسب طبقه",
    floorLabelHint: "مثلاً B1، B2، GF، ۱، M",
    parkingSlots: "تعداد جای پارک",
    shopsCount: "تعداد دکان‌ها",
    monthlyRent: "اجاره ماهانه",
    defaultMonthlyRent: "اجاره پیش‌فرض",
    defaultSalePrice: "قیمت فروش پیش‌فرض",
    shopArea: "مساحت دکان",
    commercial_dashboard: "داشبورد تجاری",
    shops: "دکان‌ها",
    shop: "دکان",
    shopNo: "شماره دکان",
    rented: "اجاره داده شده",
    rentRevenue: "درآمد اجاره",
    saleRevenue: "درآمد فروش",
    totalRevenue: "کل درآمد",
    occupancy: "اشغال",
    assignClient: "اختصاص مشتری",
    recordPayment: "ثبت پرداخت",
    payments: "پرداخت‌ها",
    payment: "پرداخت",
    rentPayment: "پرداخت اجاره",
    salePayment: "پرداخت فروش",
    deposit: "ودیعه",
    other: "سایر",
    type: "نوع",
    startDate: "تاریخ شروع",
    markAvailable: "آزاد کردن",
    parking_management: "مدیریت پارکینگ",
    slotNo: "شماره جای پارک",
    assigned: "اختصاص یافته",
    installments: "اقساط",
    installmentsCount: "تعداد اقساط",
    noInstallments: "بدون قسط",
    monthlyInstallment: "قسط ماهانه",
    installmentSchedule: "جدول اقساط",
    dueDate: "تاریخ سررسید",
    markPaid: "علامت پرداخت",
    paidPercent: "درصد پرداخت",
    soldApartments: "آپارتمان‌های فروخته شده",
    availableApartments: "آپارتمان‌های موجود",
    paidFromSales: "پرداخت شده از فروشات",
    pendingFromSales: "باقی مانده از فروشات",
    perBuilding: "به تفکیک ساختمان",
    shopRevenue: "درآمد دکان‌ها",
    companySettings: "تنظیمات شرکت",
    companyName: "نام شرکت",
    companyLogo: "لوگو",
    companyAddress: "آدرس",
    companyPhone: "شماره تماس",
    companyPhone2: "شماره تماس دوم",
    companyEmail: "ایمیل",
    companyWebsite: "ویب‌سایت",
    registrationNo: "شماره ثبت",
    footerNote: "یادداشت پاورقی",
    uploadLogo: "آپلود لوگو",
    removeLogo: "حذف لوگو",
    receipt: "رسید پرداخت",
    receiptNo: "شماره رسید",
    print: "چاپ",
    customerCopy: "نسخه مشتری",
    companyCopy: "نسخه شرکت",
    signature: "امضا",
    customerSignature: "امضای مشتری",
    authorizedSignature: "امضای مسئول",
    paymentDetails: "جزئیات پرداخت",
    installmentNo: "شماره قسط",
    upfrontPayment: "پرداخت اولیه",
    thankYou: "از پرداخت شما سپاسگزاریم",
    printReceipt: "چاپ رسید",
  },
  en: {
    appName: "Construction & Real Estate",
    appShort: "CRE",
    dashboard: "Dashboard",
    construction: "Construction",
    realEstate: "Real Estate",
    support: "Resident Support",
    materials: "Materials",
    workers: "Workers",
    salaries: "Salaries",
    expenses: "Expenses",
    apartments: "Apartments",
    clients: "Clients",
    sales: "Sales",
    requests: "Service Requests",
    settings: "Settings",
    language: "Language",
    currency: "Currency",
    add: "Add",
    edit: "Edit",
    delete: "Delete",
    save: "Save",
    cancel: "Cancel",
    confirm: "Confirm",
    name: "Name",
    description: "Description",
    quantity: "Quantity",
    unit: "Unit",
    price: "Price",
    total: "Total",
    date: "Date",
    status: "Status",
    actions: "Actions",
    search: "Search",
    noData: "No data available",
    project: "Project",
    phone: "Phone",
    address: "Address",
    role: "Role",
    dailyWage: "Daily Wage",
    daysWorked: "Days Worked",
    paid: "Paid",
    pending: "Pending",
    category: "Category",
    amount: "Amount",
    block: "Block",
    floor: "Floor",
    apartmentNo: "Apartment No",
    rooms: "Rooms",
    area: "Area (sqm)",
    available: "Available",
    sold: "Sold",
    reserved: "Reserved",
    client: "Client",
    apartment: "Apartment",
    salePrice: "Sale Price",
    paidAmount: "Paid Amount",
    remaining: "Remaining",
    issue: "Issue",
    priority: "Priority",
    assignedTo: "Assigned To",
    open: "Open",
    inProgress: "In Progress",
    completed: "Completed",
    low: "Low",
    medium: "Medium",
    high: "High",
    welcome: "Welcome",
    overview: "Overview",
    totalMaterials: "Total Materials",
    totalWorkers: "Total Workers",
    totalApartments: "Total Apartments",
    totalSales: "Total Sales",
    openRequests: "Open Requests",
    monthlyExpenses: "Monthly Expenses",
    recentActivity: "Recent Activity",
    new: "New",
    none: "None",
    inStock: "In Stock",
    supplier: "Supplier",
    notes: "Notes",
    paymentMethod: "Payment Method",
    cash: "Cash",
    bank: "Bank",
    purchaseDate: "Purchase Date",
    hireDate: "Hire Date",
    period: "Period",
    payDate: "Pay Date",
    selectWorker: "Select Worker",
    selectClient: "Select Client",
    selectApartment: "Select Apartment",
    requestedBy: "Requested By",
    deleteConfirm: "Are you sure you want to delete this item?",
    buildings: "Buildings",
    building: "Building",
    buildingName: "Building Name",
    floorsCount: "Number of Floors",
    floorPurpose: "Floor Purpose",
    flatsPerFloor: "Flats per Floor",
    bathrooms: "Bathrooms",
    kitchens: "Kitchens",
    livingRooms: "Living Rooms",
    bedrooms: "Bedrooms",
    residential: "Residential",
    parking: "Parking",
    commercial: "Commercial",
    storage: "Storage",
    amenity: "Amenity",
    mixed: "Mixed",
    generateBuilding: "Generate Full Building",
    floorConfiguration: "Floor Configuration",
    flatConfiguration: "Flat Configuration",
    defaultArea: "Default Area",
    defaultPrice: "Default Price",
    apply: "Apply",
    floorNumber: "Floor Number",
    flats: "Flats",
    addBuilding: "Add Building",
    totalFlats: "Total Flats",
    available_units: "Available Units",
    viewFlats: "View Flats",
    flatTypes: "Flat Types",
    flatType: "Flat Type",
    addFlatType: "Add Flat Type",
    typeName: "Type Name (e.g. 3-Bedroom)",
    count: "Count",
    remove: "Remove",
    noFlatTypes: "No flat types defined for this floor",
    applyToAll: "Apply to all floors",
    appliedToAll: "Settings applied to all floors",
    floorLabel: "Floor Label",
    floorLabelHint: "e.g. B1, B2, GF, 1, M",
    parkingSlots: "Parking Slots",
    shopsCount: "Number of Shops",
    monthlyRent: "Monthly Rent",
    defaultMonthlyRent: "Default Monthly Rent",
    defaultSalePrice: "Default Sale Price",
    shopArea: "Shop Area",
    commercial_dashboard: "Commercial Dashboard",
    shops: "Shops",
    shop: "Shop",
    shopNo: "Shop No",
    rented: "Rented",
    rentRevenue: "Rent Revenue",
    saleRevenue: "Sale Revenue",
    totalRevenue: "Total Revenue",
    occupancy: "Occupancy",
    assignClient: "Assign Client",
    recordPayment: "Record Payment",
    payments: "Payments",
    payment: "Payment",
    rentPayment: "Rent Payment",
    salePayment: "Sale Payment",
    deposit: "Deposit",
    other: "Other",
    type: "Type",
    startDate: "Start Date",
    markAvailable: "Mark Available",
    parking_management: "Parking Management",
    slotNo: "Slot No",
    assigned: "Assigned",
    installments: "Installments",
    installmentsCount: "Number of Installments",
    noInstallments: "No installments",
    monthlyInstallment: "Monthly Installment",
    installmentSchedule: "Installment Schedule",
    dueDate: "Due Date",
    markPaid: "Mark Paid",
    paidPercent: "Paid %",
    soldApartments: "Sold Apartments",
    availableApartments: "Available Apartments",
    paidFromSales: "Paid from Sales",
    pendingFromSales: "Pending from Sales",
    perBuilding: "Per Building",
    shopRevenue: "Shop Revenue",
    companySettings: "Company Settings",
    companyName: "Company Name",
    companyLogo: "Logo",
    companyAddress: "Address",
    companyPhone: "Phone",
    companyPhone2: "Secondary Phone",
    companyEmail: "Email",
    companyWebsite: "Website",
    registrationNo: "Registration No",
    footerNote: "Footer Note",
    uploadLogo: "Upload Logo",
    removeLogo: "Remove Logo",
    receipt: "Payment Receipt",
    receiptNo: "Receipt No",
    print: "Print",
    customerCopy: "Customer Copy",
    companyCopy: "Company Copy",
    signature: "Signature",
    customerSignature: "Customer Signature",
    authorizedSignature: "Authorized Signature",
    paymentDetails: "Payment Details",
    installmentNo: "Installment No",
    upfrontPayment: "Upfront Payment",
    thankYou: "Thank you for your payment",
    printReceipt: "Print Receipt",
  },
} as const;

export type TranslationKey = keyof typeof translations.fa;

interface I18nCtx {
  lang: Lang;
  setLang: (l: Lang) => void;
  currency: Currency;
  setCurrency: (c: Currency) => void;
  t: (k: TranslationKey) => string;
  dir: "rtl" | "ltr";
  formatMoney: (n: number) => string;
}

export const I18nContext = createContext<I18nCtx | null>(null);

export const useI18n = () => {
  const ctx = useContext(I18nContext);
  if (!ctx) throw new Error("useI18n must be used inside I18nProvider");
  return ctx;
};
